 ...
 * Production-Grade Monthly Fee Tracking (Admin)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_payment'])) {
    ...
    try {
        ...
        $postMonth = isset($_POST['month']) ? max(1, min(12, (int)$_POST['month'])) : (int)date('n');
        $postYear = isset($_POST['year']) ? max(2000, (int)$_POST['year']) : (int)date('Y');
        $monthPaidFor = date('F Y', mktime(0, 0, 0, $postMonth, 1, $postYear));
        $status = in_array($_POST['status'], ['completed', 'pending', 'failed']) ? $_POST['status'] : 'pending';
        $amount = (float)$_POST['amount'];
        $proofPath = null;
        
        // Handle file upload
        if (isset($_FILES['proof_of_payment']) && $_FILES['proof_of_payment']['size'] > 0) {
            ...
            if (!in_array($file['type'], $allowedTypes)) {
                throw new Exception("Only JPG, PNG, PDF files allowed");
            }
            ...
            $filename = 'payment_' . $studentId . '_' . time() . '.' . $ext;
        ...
        $checkSql = "SELECT payment_id, notes FROM payments WHERE student_id = ? AND month_paid_for = ?";
        $checkStmt = $pdo->prepare($checkSql);
        $checkStmt->execute([$studentId, $monthPaidFor]);
        $payment = $checkStmt->fetch();
        $notesToSave = $payment['notes'] ?? null;

        if ($proofPath) {
            $notesToSave = mergeProofPathIntoNotes($notesToSave, $proofPath);
        }
        
        if ($payment) {
            // Update existing payment
            $updateSql = "UPDATE payments SET status = ?, amount = ?, payment_date = CURDATE(), month_paid_for = ?, recorded_by = ?, notes = ?";
            $updateParams = [$status, $amount, $monthPaidFor, $_SESSION['user_id'] ?? null, $notesToSave];
            
            $updateSql .= " WHERE payment_id = ?";
            $updateParams[] = $payment['payment_id'];
            
            $updateStmt = $pdo->prepare($updateSql);
            $updateStmt->execute($updateParams);
        } else {
            // Create new payment
            $insertSql = "INSERT INTO payments (student_id, status, amount, payment_date, month_paid_for, recorded_by, notes) 
                         VALUES (?, ?, ?, CURDATE(), ?, ?, ?)";
            $insertStmt = $pdo->prepare($insertSql);
            $insertStmt->execute([$studentId, $status, $amount, $monthPaidFor, $_SESSION['user_id'] ?? null, $notesToSave]);
        }
        
        // Redirect with success message
        header("Location: {$_SERVER['REQUEST_URI']}?success=payment_updated");
        exit;
...
// Display success message if payment was updated
$successMsg = '';
if (isset($_GET['success']) && $_GET['success'] === 'payment_updated') {
    $successMsg = '<div style="background: rgba(16, 185, 129, 0.1); border: 1px solid #10b981; color: #059669; padding: 12px 16px; border-radius: 6px; margin-bottom: 20px;">
        <i class="fas fa-check-circle"></i> Payment updated successfully!
    </div>';
}

// Fetch ALL students with their payments - SIMPLE VERSION
try {
    $sql = "
        SELECT 
            s.student_id,
        ...
        LEFT JOIN payments pay ON s.student_id = pay.student_id AND pay.month_paid_for = ?
    ";
    ...
    foreach ($allStudents as $student) {
        if ($student['payment_id']) {  // Has payment record
...
<body>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <?php if (!$student['payment_id'] || $status !== 'completed'): ?>
                                                <button onclick="updatePayment(<?= $student['student_id'] ?>, '<?= htmlspecialchars($student['student_name'], ENT_QUOTES) ?>', 'completed', <?= $amount >0 ? $amount : 150 ?>)">
                                                    <i class="fas fa-check"></i> Mark as Completed
                                                </button>
                                            <?php endif; ?>
                                            <button onclick="editPayment(<?= $student['student_id'] ?>, '<?= htmlspecialchars($student['student_name'], ENT_QUOTES) ?>', <?= $amount >0 ? $amount : 150 ?>, '<?= htmlspecialchars($proofPath ?? '', ENT_QUOTES) ?>', <?= $student['payment_id'] ?? 0 ?>)">
    ...
    <script>
        ...
        function viewHistory(studentId) {
            // Close dropdown
            document.querySelectorAll('.dropdown.active').forEach(d => d.classList.remove('active'));
            window.location.href = '<?= SITE_URL ?>/admin/payment_history.php?student_id=' + studentId;
        }
    </script>
</body>
</html>
