<?php
/**
 * SelamatRide SmartSchoolBus - Edit Bus
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($formData['bus_number'])) {
            ...
            $stmt = $pdo->prepare("SELECT bus_id FROM buses WHERE bus_number = ? AND bus_id != ?");
            $stmt->execute([$formData['bus_number'], $busId]);
            if ($stmt->fetch()) {
        ...
        if (empty($formData['license_plate'])) {
            ...
            $stmt = $pdo->prepare("SELECT bus_id FROM buses WHERE license_plate = ? AND bus_id != ?");
            $stmt->execute([$formData['license_plate'], $busId]);
            if ($stmt->fetch()) {
        ...
        if (!empty($formData['device_id'])) {
            $stmt = $pdo->prepare("SELECT bus_id FROM buses WHERE device_id = ? AND bus_id != ?");
            $stmt->execute([$formData['device_id'], $busId]);
            if ($stmt->fetch()) {
        ...
        if (empty($errors)) {
            try {
                ...
                header('Location: ' . SITE_URL . '/admin/buses.php?success=updated');
                exit;
            } catch (Exception $e) {
                error_log("Bus Update Error: " . $e->getMessage());
                $errors[] = 'Failed to update bus. Please try again.';
            }
        }
        
        // Update bus data with form data for display
        $bus = array_merge($bus, $formData);
    }
}
...
<body>
    ...
    <main class="main-content" id="mainContent">
        ...
        <div class="page-header">
            <div>
                <h1>Edit Bus</h1>
                <p>Update bus information and assignments</p>
            </div>
        </div>
        ...
        <div class="form-container">
            <div class="form-header">
                <h2><i class="fas fa-bus"></i> Edit Bus: <?= htmlspecialchars($bus['bus_number']) ?></h2>
                <p>Update the information below to modify this bus</p>
            </div>
            ...
            <div class="info-box">
                ...
                <div class="info-box-content">
                    <strong>Bus ID: #<?= $bus['bus_id'] ?></strong>
                    <p>Created: <?= date('M d, Y h:i A', strtotime($bus['created_at'])) ?></p>
                    <?php if ($bus['updated_at']): ?>
                        <p>Last Updated: <?= date('M d, Y h:i A', strtotime($bus['updated_at'])) ?></p>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="form-grid">
                        ...
                        <select id="assigned_driver_id" name="assigned_driver_id" class="form-control">
                            ...
                            <?php foreach ($drivers as $driver): ?>
                                <option value="<?= $driver['user_id'] ?>" 
                                    <?= $bus['assigned_driver_id'] == $driver['user_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            value="<?= htmlspecialchars($bus['device_id'] ?? '') ?>"
                        >
                    ...
                    <div class="form-group full-width">
                        ...
                        ><?= htmlspecialchars($bus['route_description'] ?? '') ?></textarea>
